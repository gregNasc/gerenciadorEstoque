...
if settings.DEBUG:
    ...
    # Em instalações com volume persistente, o próprio Django entrega somente
    # fotos e QR codes públicos. DANFEs, declarações, mensagens e anexos não
    # ficam expostos por /media: continuam passando pelas views autorizadas.
    from django.views.static import serve

    media_prefix = settings.MEDIA_URL.lstrip('/').rstrip('/')
    urlpatterns += [
        re_path(
            rf'^{media_prefix}/(?P<path>(?:equipamentos|qrcodes)/.*)$',
            serve,
